 ...
 * PZSD lookup verbatim; any other slug resolves a versioned norm_sets row
 * (docs/06). Which version bundle gets scored is selected by `$productCode`
 * (ProductCatalog) — the engine has no hardcoded product.
 */
class InterpreterEngine implements ScoringEngine
{
    ...
    public function score(array $registration, array $tools, array $scopes = ['full'], string $normSet = 'male-legacy', string $format = 'keys', string $productCode = ProductCatalog::DEFAULT_CODE, ?AuditCollector $audit = null, ?NormSampler $sampler = null): array
    {
        ...
        // The norm set selects the raw→normed conversion table (docs/06).
        // Legacy sets reproduce the gender-split PZSDConversionMatrix
        // lookup exactly; versioned sets load their own entry rows. Session
        // rows are tagged with the registrant's gender (defaulting to the
        // set's gender for the legacy sets) — the tag no longer selects the
        // norm table, the set does.
        $normTable = $this->config->normTable($normSet);
        $gender = match (strtoupper($registration['gender'] ?? '')) {
            'M', 'F' => strtoupper($registration['gender']),
            default => match ($normSet) {
                'female-legacy' => 'F',
                default => 'M',
            },
        };
        ...
        $interpreter = new RuleInterpreter($this->config, $state, $audit, $normTable, $sampler);
